...
<body>
	<div class="container">
		...
		<table>
		<thead>
			<tr><td>No</td><td>책이름</td><td>저자</td><td>출판사</td><td>가격</td><td>조회수</td><td>비고</td></tr>
		</thead>
		...
		<tbody>
			<tr>
				...
				<td>
					...
					<a href="Retrieve.do?bnum=${my.bnum }">${my.bname }</a>
				</td>
				<td>${my.bauthor }</td>
				<td>${my.bpublisher }</td>
				<td><fmt:formatNumber value="${my.bprice }" type="number"/></td>
				<td>${my.breadcnt }</td>
				<td><a href="BookModify.jsp?bnum=${my.bnum }">수정</a></td>
			</tr>
		</tbody>
		</c:forEach>
	</table>
